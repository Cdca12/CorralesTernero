...
AS
	...
	--DECLARE @Username varchar(255) = 'usrAdministrador' --Test
...
DECLARE @ConfiguracionMenu TABLE (
	MenuID int,
	MenuName varchar(max),
	MenuItemID int,
	MenuItemName varchar(max),
	Username varchar(max)
)
INSERT @ConfiguracionMenu EXECUTE spObtenerConfiguracion 'usrAdministrador'
SELECT MenuID, MenuName FROM @ConfiguracionMenu 
GROUP BY MenuID, MenuName


EXECUTE spObtenerConfiguracion 'usrAdministrador'
...
AS
	...
	INSERT @Configuracion EXECUTE spObtenerConfiguracion 'usrAdministrador'
	SELECT MenuID, MenuName FROM @ConfiguracionMenu 
	GROUP BY MenuID, MenuName
